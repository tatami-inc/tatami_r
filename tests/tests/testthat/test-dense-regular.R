...
for (dims in list(c(0, 10), c(10, 0))) {
    NR <- dims[1]
    NC <- dims[2]
    mat <- RegularChunkedMatrix(matrix(rpois(NR * NC, lambda=2), nrow=NR, ncol=NC), chunks=c(NR, NC))

    test_that("dense empty regular-chunked integer matrix passes basic checks", {
        expect_s4_class(chunkGrid(mat), "RegularArrayGrid")
        expect_identical(type(mat), "integer")
